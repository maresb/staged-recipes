...
# 1. Payload: every non-dist-info file in the wheel must be byte-identical to
#    what this package installed, with nothing missing or extra.
...
# 2. Metadata: the fields that affect installers and resolvers must match.
...
for field, transform in [
    ...
    a = sorted(map(transform, theirs.get_all(field, [])) if transform else theirs.get_all(field, []))
    b = sorted(map(transform, mine.get_all(field, [])) if transform else mine.get_all(field, []))
    if a != b:
...
if errors:
    ...
    sys.exit(1)
print(f"OK: installed package matches the upstream wheel ({len(wheel_files)} payload files)")
